 ...
 *
 */
bool CcnContext::isAddressFamily(unsigned short af) {
 ...
 *
 */
MACAddress CcnContext::getSrcAddress802() {
    if (src.sa.sa_family != AF_PACKET) {
        DBGPRT(AUX, Err, "CcnContext::getSrcAddress802()")
                << " CcnContext src.sa.sa_family != AF_PACKET " << std::endl;
        opp_error(
                "Error: failed attempt to access src address type in CcnContext as AF_PACKET type!");
    }
 ...
 *
 */
MACAddress CcnContext::getDstAddress802() {
    if (dst.sa.sa_family != AF_PACKET) {
        DBGPRT(AUX, Err, "CcnContext::getDstAddress802()")
                << " CcnContext dst.sa.sa_family != AF_PACKET" << std::endl;
        opp_error(
                "Error: failed attempt to access dst address type in CcnContext as AF_PACKET type!");
    }
 ...
 *
 */
L3Address CcnContext::getSrcAddressInet() {
 ...
 *
 */
L3Address CcnContext::getDstAddressInet() {
    L3Address inAddr;
    ...
    if (dst.sa.sa_family == AF_INET) {
        ...
        DBGPRT(AUX, Err, "CcnContext::getDstAddressInet()")
                << " CcnContext dst.sa.sa_family != AF_INET|AF_INET6 "
                << std::endl;
        opp_error(
                "Error: failed attempt to access dst address type in CcnContext as AF_INET(6) type!");
    }
...
int CcnContext::getSrcPort() {
    if (src.sa.sa_family == AF_INET) {
        ...
        DBGPRT(AUX, Err, "CcnContext::getSrcPort()")
                ...
                << std::endl;
        opp_error(
                "Error: failed attempt to access src address type in CcnContext as AF_INET(6) socket type!");
    }
...
int CcnContext::getDstPort() {
    if (dst.sa.sa_family == AF_INET) {
        ...
        DBGPRT(AUX, Err, "CcnContext::getDstPort()")
                ...
                << std::endl;
        opp_error(
                "Error: failed attempt to access dst address type in CcnContext as AF_INET(6) socket type!");
    }
 ...
 *
 */
void CcnContext::set802Info(const MACAddress &srcMac,
        const MACAddress &dstMac) {
    if ((src.sa.sa_family != AF_PACKET) || (dst.sa.sa_family != AF_PACKET)) {
        DBGPRT(AUX, Err, "CcnContext::set802Info()")
                << " Pre-initialised CcnContext for address family != AF_PACKET "
                << std::endl;
        opp_error(
                "Error: failed attempt to set address information in CcnContext previously initialised as AF_PACKET type!");
    }
 ...
 *
 */
void CcnContext::set802Info(const struct sockaddr_ll &srcSockLL,
        const struct sockaddr_ll &dstSockLL) {
    if ((src.sa.sa_family != AF_PACKET) || (dst.sa.sa_family != AF_PACKET)) {
        DBGPRT(AUX, Err, "CcnContext::set802Info()")
                << " Pre-initialised CcnContext for address family != AF_PACKET "
                << std::endl;
    ...
    if ((srcSockLL.sll_family != AF_PACKET)
            || (dstSockLL.sll_family != AF_PACKET)) {
        DBGPRT(AUX, Err, "CcnContext::set802Info()")
                << " The provided CcnContext address information is invalid ( != AF_PACKET type)"
                << std::endl;
        opp_error(
                "Error: trying to set CcnContext with invalid address information (!= AF_PACKET type) !");
    }
 ...
 *
 */
void CcnContext::set802Info(const char *srcMac, const char *dstMac) {
    if ((src.sa.sa_family != AF_PACKET) || (dst.sa.sa_family != AF_PACKET)) {
        DBGPRT(AUX, Err, "CcnContext::set802Info()")
                << " Pre-initialised CcnContext for address family != AF_PACKET "
                << std::endl;
 ...
 *
 */
void CcnContext::setInetInfo(const L3Address &srcIp, const L3Address &dstIp,
    ...
    if (!((src.sa.sa_family == AF_INET) && (dst.sa.sa_family == AF_INET))
        ...
        DBGPRT(AUX, Err, "CcnContext::setInetInfo()")
                << " Pre-initialised CcnContext for address family != AF_INET(6) "
                << std::endl;
        opp_error(
                "Error: failed attempt to set address information in CcnContext previously initialised as AF_INET(6) type!");
    }

    if (!srcIp.getType() == L3Address::IPv6 && !dstIp.getType() == L3Address::IPv6 ) {

        IPv4Address *srcIp = dynamic_cast<IPv4Address *>(srcIp);
        IPv4Address *dstIp = dynamic_cast<IPv4Address *>(dstIp);

        src.in4.sin_addr.s_addr = htonl(srcIp->getNetwork().getInt());
        dst.in4.sin_addr.s_addr = htonl(dstIp->getNetwork().getInt());
        src.in4.sin_port = htons(srcPort);
        dst.in4.sin_port = htons(dstPort);
    } else if (srcIp.getType() == L3Address::IPv6 && dstIp.getType() == L3Address::IPv6) {
        opp_error("CcnContext::setInetInfo:: IPv6");
...
//        dst.in6.sin6_port = htons(dstPort);
        ...
        DBGPRT(AUX, Err, "CcnContext::setInetInfo()")
                << " The provided CcnContext address information is invalid (failed to identify address family as AF_INET|AF_INET6)"
                << std::endl;
        opp_error(
                "Error: trying to set CcnContext with invalid address information (failed to identify address family)!");
    }
 ...
 *
 */
void CcnContext::setInetInfo(const struct sockaddr &srcSockInet,
        const struct sockaddr &dstSockInet) {
    if ((srcSockInet.sa_family == AF_INET)
            && (dstSockInet.sa_family == AF_INET)) {
        if ((src.sa.sa_family != AF_INET) || (dst.sa.sa_family != AF_INET)) {
            DBGPRT(AUX, Err, "CcnContext::setInetInfo()")
                    << " Pre-initialised CcnContext for address family != AF_INET "
                    << std::endl;
            opp_error(
                    "Error: failed attempt to set address information in CcnContext previously initialised as AF_INET type!");
        }
    ...
    } else if ((srcSockInet.sa_family == AF_INET6)
            && (dstSockInet.sa_family == AF_INET6)) {
        if ((src.sa.sa_family != AF_INET6) || (dst.sa.sa_family != AF_INET6)) {
            DBGPRT(AUX, Err, "CcnContext::setInetInfo()")
                    << " Pre-initialised CcnContext for address family != AF_INET6 "
                    << std::endl;
            opp_error(
                    "Error: failed attempt to set address information in CcnContext previously initialised as AF_INET6 type!");
        }
 ...
 */
void CcnInet::initialize() {
    bool iSetGlobalDbgLvl = false;
    bytes_ = 0;

    if(thorugput_timer == NULL){
           thorugput_timer = new cMessage("Throughput_Timer");

           windows_thr = 0.5;
           string thr = "throughput_";
           thr += getParentModule()->getParentModule()->getName();
           thoughput.setName(thr.c_str());

          scheduleAt(simTime(),thorugput_timer);
       }
    ...
    (netTopology->par("auxDebug").isSet()
            && (netTopology->par("auxDebug").boolValue() == true)) ?
            DebugOutput::Init(std::cerr) : DebugOutput::Init();

    debugLevel =
            (par("debugLevel").isSet()) ?
                    par("debugLevel") : ::defaultDebugLevel;

    if (iSetGlobalDbgLvl) {
        DBGPRT(EVAUX, Info, this->getFullPath())
                << "Global default debug level is set to: "
                << ::defaultDebugLevel << std::endl;
    }

    DBGPRT(EVAUX, Info, this->getFullPath()) << " Set my debug level to "
            << debugLevel << std::endl;

    /* Set up access to NICs (Interface Table of INET framework)
     */
    nicTbl = getModuleFromPar<IInterfaceTable>(par("interfaceTableModule"), this);
    forwardingTable = getModuleFromPar<ForwardingTable>(par("forwardingTableModule"), this);

    nicOutBaseGateId = gateSize("ifOut") == 0 ? -1 : gate("ifOut", 0)->getId();
    ...
    if (0 /*TODO: check that we have UDP/IP layer below */) {
        ...
        DBGPRT(EVAUX, Info, this->getFullPath())
                << " Initialised UDP tunnels for socket Faces" << std::endl;
    }
...
int CcnInet::addToCacheDummy(const char *contentName, const int seqNumStart,
        const int numChunks) {
...
//    if (par("ospfn")) {
//
//        OSPFNPrefixAdv* packet = new OSPFNPrefixAdv();
//        packet->setKind(PREFIX_ADVERTISE);
//        packet->setPrefixName(contentName);
//
//        send(packet, "ospfnOut");
//    }
...
//
//    DBGPRT(EVAUX, Info, this->getFullPath())
//            << " Adding dummy content in my cache: " << contentName << "/["
//            << seqNumStart << "-" << seqNumStart + numChunks << "]"
//            << std::endl;
 ...
 */
void CcnInet::handleMessage(cMessage *msg) {
    /**
...
void CcnInet::handleSelfMessage(cMessage *msg){

if(msg == thorugput_timer){

       if( simTime() - last_record > windows_thr){

           double thr = bytes_/(windows_thr)*8/1000000;
           bytes_ = 0;
           last_record = simTime();
           thoughput.record(thr);
       }
       scheduleAt(simTime()+windows_thr,thorugput_timer);
   }
...
CcnInet::netIfFromMac(const MACAddress &macAddr) {
    ...
    if ((numNics < 1) || (gateSize("ifOut") == 0)) {
        DBGPRT(AUX, Err, this->getFullPath()) << " No Gates to the Link Layer"
                << std::endl;
        return 0;
    }
    ...
    DBGPRT(AUX, Err, this->getFullPath()) << " Could not resolve MAC Address "
            << macAddr.str() << " to an existing NIC (InterfaceEntry *)"
            << std::endl;

    return 0;
}

MACAddress CcnInet::macFromnetIf(int ifId) {
    ...
    for (int i = 0; i < numNics; i++) {
        nic = nicTbl->getInterface(i);
        if(nic->getInterfaceId() == ifId)
            return nic->getMacAddress();
    }

    DBGPRT(AUX, Err, this->getFullPath()) << " Could not resolve IFid "
            << ifId << " to an existing MACAddress"
            << std::endl;
 ...
 */
int CcnInet::netIfIndexFromMac(const MACAddress &macAddr) {
    InterfaceEntry *nic = 0;
...
void CcnInet::registerApp(int app_id, cGate *srcGate){

    std::cout << getParentModule()->getParentModule()->getName() << endl;
    cGate * destGate = srcGate->getPathStartGate();
    appTable.insert(std::pair<int,int>(app_id,destGate->getId()));
}

void CcnInet::registerChunkListener(chunk_t prefix, int appId){

    chunkListeners.insert(std::pair<chunk_t,int>(prefix,appId));
}

//TODO Implementar expressões regulares para verificar content names..
//por enquanto, para pegar qualquer chunk, basta registrar o nome "*"
int CcnInet::getChunkListener(chunk_t prefix){

    if(!chunkListeners.size())
        return -1;

       // ChunkListener::iterator it = chunkListeners.begin();

       //   std::string content = contentName;
        ChunkListener::iterator listener;
        bool finded = false;
        for (;;) {

            listener = chunkListeners.find(prefix);
              if (listener != chunkListeners.end()) {
                  finded = true;
                  break;
              }

              prefix = Util::getPrefixContentName(prefix);

            if (prefix == "") {
                listener = chunkListeners.find("*");
...
int CcnInet::getAppGate(int app_id){

    AppTable::iterator it = appTable.begin();

//    std::cerr << "CcnInet::getAppGate: ask for: " << app_id << std::endl;
//    std::cerr << "<AppTable>" << std::endl;
//    for(; it != appTable.end(); it++){
//        std::cerr << " first: " << (*it).first << " second: " << (*it).second << std::endl;
//    }

    it = appTable.find(app_id);
 ...
 */
int CcnInet::gateIdFromMac(const MACAddress &macAddr) {
    int gateId = -1;
    InterfaceEntry *nic = 0;
    ...
    DBGPRT(AUX, Info, this->getFullPath()) << "The gate for MAC addr "
            << macAddr.str() << " is " << gateId << std::endl;
 ...
 */
UDPSocket *
CcnInet::whichUdpSocket(const L3Address &address, const int port) {
    for (int i = 0; i < numUdpFaces; i++) {
 ...
 */
void CcnInet::updateDisplayString() {
    char buf[100] = "";
    if (inPktsDropped > 0)
        sprintf(buf + strlen(buf), "(In)DROPPED:%d ", inPktsDropped);
    if (outPktsDropped > 0)
        sprintf(buf + strlen(buf), "(Out)DROPPED:%d ", outPktsDropped);
    if (inPkts > 0)
        sprintf(buf + strlen(buf), "RECEIVED:%d ", inPkts);
    if (outPkts > 0)
        sprintf(buf + strlen(buf), "SENT:%d ", outPkts);
 ...
 */
void CcnInet::toLowerLayer(cMessage *msg, double delayed) {
    ...
    if (ccnCtx->isAddressFamily(AF_INET) || ccnCtx->isAddressFamily(AF_INET6)) {
        ...
        UDPSocket *s = whichUdpSocket(ccnCtx->getSrcAddressInet(),
                ccnCtx->getSrcPort());

        if (s) {
            s->sendTo(ccnPkt, ccnCtx->getDstAddressInet(),
                    ccnCtx->getDstPort());

            DBGPRT(EVAUX, Info, this->getFullName())
                    << "Sending CcnPacket over UDP socket" << std::endl;
            ...
            DBGPRT(AUX, Detail, this->getFullName())
                    ...
                    << ccnCtx->getSrcAddressInet() << ":"
                    << ccnCtx->getSrcPort() << std::endl;
        ...
        //FIXME[EAMA] fix to "setControlInfo(): message already has control info attached."
        if (ccnPkt->getControlInfo())
            delete ccnPkt->removeControlInfo();
        ...
        if (ccnCtx->getSrcAddress802().isBroadcast()) {
            ...
            if ((numNICs < 1) || (outGateSize == 0)) {
                DBGPRT(AUX, Err, this->getFullPath())
                        << "No Gates to the Link Layer" << std::endl;
            ...
            if (numNICs != outGateSize) {
                DBGPRT(AUX, Warn, this->getFullPath()) << "Number of NICs = "
                        << numNICs << ", size of eth[] vector = " << outGateSize
                        << ". This might create issues!" << std::endl;
            }
            ...
            DBGPRT(EVAUX, Info, this->getFullName())
                    << "Broadcasting CcnPacket over all Link Layer NICs"
                    << std::endl;
            goto CLEANUP_AND_EXIT;
        }
        ...
        if (nicGateId != -1) {
             ...
             * Found NIC to use
             */
            sendDelayed(ccnPkt, delayed, nicGateId);

            DBGPRT(EVAUX, Info, this->getFullName())
                    << "Sending CcnPacket via Link Layer NIC, with MAC ID="
         ...
         */
        DBGPRT(EVAUX, Err, this->getFullName())
                << "Invalid CcnContext. Drop silently" << std::endl;
        DBGPRT(AUX, Detail, this->getFullName())
                << "CcnContext not configured with AF_INET or AF_PACKET address family."
                << " Cannot decide how to forward the packet" << std::endl;

        outPktsDropped++;
        goto CLEANUP_AND_EXIT;
    }

    CLEANUP_AND_EXIT: if (ccnCtx)
        delete ccnCtx;
    if (msgDrop)
        delete msg;
    if (ev.isGUI())
 ...
 ****************************************************************************/
void CcnInet::toUpperLayer(cMessage *msg) {
    CcnAppMessage *appmsg = dynamic_cast<CcnAppMessage *>(msg);
     ...
     * O registro usa o trecho de código abaixo e permite criar
     * o mapeamento porta/aplicação. A fim de enviar o pacote para aplicação certa.
     """Ccn* ccn = getModuleFromPar<Ccn>(par("ccnd"), this);"""
     """cGate *mygate = gate("ipIn");"""
     """ccn->registerApp(appId,mygate);"""
     * */

    int appid = appmsg->getAppId();
    if(appid){
        int AppPort = getAppGate(appid);
        if(AppPort > 0){
            cGate* cgate = gate(AppPort);
            send(appmsg, cgate);
        }else {
            std::cout << getParentModule()->getParentModule()->getName() << endl;
            getAppGate(-1);
            error("CcnInet::toUpperLayer: Have you register your cnn aplication? or have you setting the app id into CcnAppMessage?");
        }
    }else{
        error("AppId = 0, CcnInet::toUpperLayer: Have you register your cnn aplication? or have you setting the app id into CcnAppMessage?");
    }
 ...
 */
void CcnInet::endService(cPacket *msg) {
    CcnContext *ccnCtx = NULL;
    bool msgDrop = false;
...
//    if (msg->getArrivalGate()->isName("ospfnIn")) {
//
//        OSPFNPrefixAdv *packet = dynamic_cast<OSPFNPrefixAdv *>(msg);
//
//        if (packet) {
//            handlerOSPFNPrefixAdv(packet);
//        } else
//            handlerUpLayerProtocol(msg);
//        //handlerOSPFNtoIPLayer(msg);
//
//    } else
        ...
        if (msg->arrivedOn("ifIn")) {
            ...
            InterfaceEntry *intf = inGate ?
                    nicTbl->getInterfaceByNetworkLayerGateIndex(inGate->getIndex()) : NULL;

            if (intf->getMTU() < msg->getByteLength()) {

                DBGPRT(EVAUX, Err, this->getFullName())
                        << "Received a too big CcnPacket from layer below. Drop silently"
                        << std::endl;
                DBGPRT(AUX, Detail, this->getFullName()) << this->getFullName()
             ...
             */
            Ieee802Ctrl *macCtx = check_and_cast<Ieee802Ctrl *>(
            ...
            //to OSPFN:
            //ccnCtx->set802Info( macCtx->getSrc(), intf->getMacAddress());
            ...
            case UDP_I_ERROR:
                ...
                errInfo = check_and_cast<UDPErrorIndication *>(
                        msg->removeControlInfo());
            ...
            case UDP_I_DATA:
                ...
                udpCtx = check_and_cast<UDPDataIndication *>(
                        msg->removeControlInfo());

                /* Create CcnContext with endpoint information and attach to message
                 */
                if (udpCtx->getSrcAddr().getType() == L3Address::IPv6 )
                ...
                ccnCtx->setInetInfo(udpCtx->getSrcAddr(), udpCtx->getDestAddr(),
                        udpCtx->getSrcPort(), udpCtx->getDestPort());
                delete udpCtx;
            ...
            default:
                 ...
                 */
                DBGPRT(EVAUX, Info, this->getFullName())
                        << "Invalid incoming UDP Packet. Drop silently"
                        << std::endl;
                DBGPRT(AUX, Detail, this->getFullName())
                        << "\t received message, has invalid code: getKind()="
                        << msg->getKind() << std::endl;
...
//                send(decapsulate(datagram, macCtx), "ospfnOut");
//            } else
//                error("CcnInet::endService: Unknown Packet arrived by ifIn.. check this out.");
//        }else
//            error("CcnInet::endService: non IPv4Datagram Packet arrived by ifIn.. check this out.");
//
//
//    }
