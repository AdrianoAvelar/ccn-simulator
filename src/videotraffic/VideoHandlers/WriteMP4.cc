...
#include "Misc.h"
//
//static void write_sample(GF_ISOSample *smp, GF_AVCConfig *avccfg,
//        GF_M4ADecSpecInfo *acfg, u32 is_aac, u32 aac_type, GF_BitStream *bs) {
//
//    if (!avccfg) {
//        if (is_aac) {
...
//        //original u8 *ptr = smp->data;
//        char *ptr = smp->data;
//        while (remain) {
//            nal_size = 0;
//            for (j = 0; j < avccfg->nal_unit_size; j++) {
//                nal_size |= *ptr;
//                if (j + 1 < avccfg->nal_unit_size)
//                    nal_size <<= 8;
//                remain--;
//                ptr++;
//            }
...
//}
//
//static int write_raw(GF_ISOFile *fi, u32 hint, u32 data, char *fn, data_t *D,
//        unsigned int mode) {
...
//
//    if (m_stype == GF_ISOM_SUBTYPE_MPEG4
//            || m_stype == GF_ISOM_SUBTYPE_MPEG4_CRYP)
...
//        if (is_aac == 2)
//            aac_type = acfg.base_object_type - 1;
//        free(pdsi);
//        pdsi = 0;
//    }
//    if (pdsi) {
...
//                    avccfg->sequenceParameterSets, i);
//            gf_bs_write_u32(bs, 1);
...
//        for (i = 0; i < count; i++) {
//            GF_AVCConfigSlot *sl = (GF_AVCConfigSlot*) gf_list_get(
//                    avccfg->pictureParameterSets, i);
//            gf_bs_write_u32(bs, 1);
//            gf_bs_write_data(bs, sl->data, sl->size);
//        }
//    }
//    samples = gf_isom_get_sample_count(fi,
//            !(mode & AUDIO) && (mode & FRAME || mode & COMPLETE) ? data : hint);
//    for (l = 0, i = 0; i < samples; i++) {
//        if (0
//                == (smp = gf_isom_get_sample(fi,
//                        !(mode & AUDIO) && (mode & FRAME || mode & COMPLETE) ?
//                                data : hint, i + 1, &di)))
//            goto S;
//        if (mode & AUDIO) {
//            if (!D->F[i].lost) {
//                hint_smp = gf_isom_hint_sample_new(GF_ISOM_BOX_TYPE_RTP_STSD);
//                bs_tmp = gf_bs_new(smp->data, smp->dataLength,
//                        GF_BITSTREAM_READ);
//                gf_isom_hint_sample_read(hint_smp, bs_tmp, smp->dataLength);
//                gf_bs_del(bs_tmp);
//                packets = gf_list_count(hint_smp->packetTable);
//                for (j = 0; j < packets; j++, l++) {
//                    pck = (GF_RTPPacket*) gf_list_get(hint_smp->packetTable, j);
//                    count = gf_list_count(pck->DataTable);
//                    for (k = 0; k < count; k++) {
//                        GF_GenericDTE *dte = (GF_GenericDTE *) gf_list_get(
//                                pck->DataTable, k);
//                        if (dte->source == 2) {
//                            GF_SampleDTE *sdte = (GF_SampleDTE *) dte;
//                            if (sdte->sampleNumber != last_smp) {
//                                if (last_smp != 0) {
//                                    write_sample(tmp, avccfg, &acfg, is_aac,
//                                            aac_type, bs);
//                                    gf_isom_sample_del(&tmp);
//                                }
//                                if (0
//                                        == (tmp = gf_isom_get_sample(fi, data,
//                                                sdte->sampleNumber, 0)))
//                                    goto S;
...
//                            || !(h = mark_not_coded((unsigned char*) smp->data,
//                                    smp->dataLength, dsi.NumBitsTimeIncrement)))
//                        goto E;
//                }
//                if (m_stype == 'avc1' && i == 0)
//                    h = 64;
...
//                        smp->dataLength = 8 + h;
//                    if (mode & FILL)
//                        memset(smp->data + 8 + h, 0, smp->dataLength - (8 + h));
//                }
//            }
...
//                            if (last_smp != 0) {
//                                if (!D->P[l].lost)
//                                    write_sample(tmp, avccfg, 0, 0, 0, bs);
//                                gf_isom_sample_del(&tmp);
//                            }
//                            if (0
//                                    == (tmp = gf_isom_get_sample(fi, data,
...
//                                memset(tmp->data + sdte->byteOffset + 8, 0,
//                                        sdte->dataLength - 8);
//                            }
//                        }
//                    }
...
//}
//
//static int write_mp4(GF_ISOFile *fi, u32 hint, u32 data, char *fn, data_t *D,
//        unsigned int mode) {
//    GF_ESD *esd = 0;
//    GF_ISOSample *smp = 0, *tmp = 0;
//    GF_HintSample *hint_smp;
...
//    GF_ISOFile *fo;
//
//    u32 i, j, k, l, count, ts, rate, dur, pos, di, samples, packets, last_smp =
//            0;
//    u32 h, newTk, descIndex, msubtype, trackID = gf_isom_get_track_id(fi, data);
//
//    if (0 == (fo = gf_isom_open(fn, GF_ISOM_WRITE_EDIT, 0)))
//        goto O;
//    if (GF_ISOM_SUBTYPE_MPEG4
//            == (msubtype = gf_isom_get_media_subtype(fi, data, 1))) {
//        if (0 != (esd = gf_isom_get_esd(fi, data, 1))) {
//            esd->dependsOnESID = 0;
//            esd->OCRESID = 0;
//        }
//    }
//    newTk = gf_isom_new_track(fo, trackID, gf_isom_get_media_type(fi, data),
//            gf_isom_get_media_timescale(fi, data));
...
//                gf_m4v_get_config(esd->decoderConfig->decoderSpecificInfo->data,
...
//    } else {
//        gf_isom_clone_sample_description(fo, newTk, fi, data, 1, 0, 0,
//                &descIndex);
//    }
//    pos = rate = 0;
...
//            !(mode & AUDIO) && (mode & FRAME || mode & COMPLETE) ? data : hint);
//    for (l = 0, i = 0; i < samples; i++) {
//        if (0 == (smp = gf_isom_get_sample(fi, !(mode & AUDIO) &&
//                (mode & FRAME || mode & COMPLETE) ? data : hint, i + 1, &di)))
//            goto R;
//        if (mode & AUDIO) {
//            if (!D->F[i].lost) {
//                hint_smp = gf_isom_hint_sample_new(GF_ISOM_BOX_TYPE_RTP_STSD);
...
//                                if (last_smp != 0) {
//                                    if (GF_OK
//                                            != gf_isom_add_sample(fo, newTk,
//                                                    descIndex, tmp))
//                                        goto A;
//                                    gf_isom_sample_del(&tmp);
//                                }
//
//                                if (0 == (tmp = gf_isom_get_sample(fi, data,
//                                                sdte->sampleNumber, 0)))
//                                    goto R;
//                                last_smp = sdte->sampleNumber;
//                            }
...
//                }
//            }
//            if (GF_OK != gf_isom_add_sample(fo, newTk, descIndex, smp))
//                goto A;
//        } else {
//            hint_smp = gf_isom_hint_sample_new(GF_ISOM_BOX_TYPE_RTP_STSD);
//            bs_tmp = gf_bs_new(smp->data, smp->dataLength, GF_BITSTREAM_READ);
...
//            if (esd->decoderConfig->bufferSizeDB < smp->dataLength)
//                esd->decoderConfig->bufferSizeDB = smp->dataLength;
//            if (smp->DTS - pos > ts) {
//                if (esd->decoderConfig->maxBitrate < rate)
...
//    }
//
//    if (0 == (dur = (u32) gf_isom_get_media_duration(fo, newTk)))
//        dur = ts;
...
//            gf_isom_set_pl_indication(fo, GF_ISOM_PL_VISUAL,
//                    iod->visual_profileAndLevel);
//        } else if (esd->decoderConfig->objectTypeIndication == 32) {
//            GF_M4VDecSpecInfo dsi;
//            gf_m4v_get_config(esd->decoderConfig->decoderSpecificInfo->data,
//                    esd->decoderConfig->decoderSpecificInfo->dataLength, &dsi);
...
//            gf_error_to_string(gf_isom_last_error(0)));
//    goto X;
...
//}
//
//static int write_damaged(GF_ISOFile *f, u32 hint, u32 data, char *fn, char *ext,
//        data_t *D, unsigned int mode) {
//
//    if (!write_mp4(f, hint, data, strcat(fn, ".mp4"), D, mode)) {
//        fprintf(stderr, "Couldn't export mp4-track %d: %s\n", hint,
//                gf_error_to_string(gf_isom_last_error(0)));
//    }
//    if (!write_raw(f, hint, data, (strcpy(strrchr(fn, '.') + 1, ext), fn), D,
//            mode)) {
...
//}
//
int WriteMP4(char *s, char *d, data_t *D, unsigned int mode) {
//    GF_ISOFile *fi;
...
//            if (GF_OK
//                    == gf_isom_get_reference(fi, i + 1, GF_ISOM_REF_HINT, 1,
//                            &ref_track)) {
//                if (ref_track) {
//                    type = gf_isom_get_media_type(fi, ref_track);
//                    sub = gf_isom_get_mpeg4_subtype(fi, ref_track, 1);
//                    if (!sub)
//                        sub = gf_isom_get_media_subtype(fi, ref_track, 1);
