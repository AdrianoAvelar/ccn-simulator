...
void CalcHist(dump_t *F, unsigned l, unsigned max, unsigned width, char *id)
{
  FILE *f;
  char s[32];
  unsigned i, j,*hist = ( unsigned int*)malloc(max / width + 1);
...
void PoBLoss(dump_t *pD, unsigned l, unsigned PoB, unsigned int mode)
{
...
void Hist(double *D, unsigned l, double min, double max, unsigned interv)
{
  ...
  if ((hist = (unsigned int*)calloc(interv, sizeof *hist)) == 0) {
