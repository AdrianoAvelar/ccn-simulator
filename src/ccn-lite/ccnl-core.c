   ...
   returns n>0 for matched components (CMP_MATCH, CMP_LONGEST) */
{
    ...
    fprintf(stderr, "ccnl_prefix_cmp (mode=%d, nlen=%d, plen=%d, %d), name=%s: %d (%p)\n",
			       ...
			       unsigned char **content, int *contlen)
{
    ...
    fprintf(stderr, "ccnl_extract_prefix\n");
	...
	if (typ == CCN_TT_DTAG) {
		...
		if (num == CCN_DTAG_EXCLUDE) {
		    fprintf(stderr, "warning: 'exclude' field ignored\n");
		} else
		    continue;
	    }
...
// sa!=NULL && ifndx!=-1: use this (incoming) interface for outgoing
{
    ...
    fprintf(stderr, "ccnl_get_face_or_create src=%s\n",
    ...
    fprintf(stderr, "  found suitable interface %d for %s\n", ifndx,
	     sa ? ccnl_addr2ascii((sockunion*)sa) : "(local)");
...
ccnl_face_remove(struct ccnl_relay_s *ccnl, struct ccnl_face_s *f)
{
    ...
    fprintf(stderr, "ccnl_face_remove relay=%p face=%p\n", (void*)ccnl, (void*)f);
...
{
    int j;
    fprintf(stderr, "ccnl_interface_cleanup\n");
...
ccnl_interface_CTS(void *aux1, void *aux2)
{
    ...
    fprintf(stderr, "ccnl_interface_CTS interface=%p, qlen=%d, sched=%p\n",
		       ...
		       struct ccnl_buf_s *buf, sockunion *dest)
{
    ...
    fprintf(stderr, "ccnl_interface_enqueue interface=%p buf=%p (qlen=%d)\n",
	     (void*)ifc, (void*)buf, ifc->qlen);

    if (ifc->qlen >= CCNL_MAX_IF_QLEN) {
	fprintf(stderr, "  DROPPING buf=%p\n", (void*)buf);
	ccnl_free(buf);
	return;
    }
...
ccnl_face_dequeue(struct ccnl_relay_s *ccnl, struct ccnl_face_s *f)
{
    ...
    fprintf(stderr, "ccnl_face_dequeue face=%p (id=%d.%d)\n",
...
{
    fprintf(stderr, "ccnl_face_CTS_done face=%p cnt=%d len=%d\n", ptr, cnt, len);
...
ccnl_face_CTS(struct ccnl_relay_s *ccnl, struct ccnl_face_s *f)
{
    ...
    fprintf(stderr, "ccnl_face_CTS face=%p sched=%p\n", (void*)f, (void*)f->sched);
...
ccnl_nonce_find_or_append(struct ccnl_relay_s *ccnl, struct ccnl_buf_s *nonce)
{
    ...
    int i;
    fprintf(stderr, "ccnl_nonce_find_or_append\n");
		  ...
		  int minsuffix, int maxsuffix, struct ccnl_buf_s **ppkd)
{
    ...
    fprintf(stderr, "ccnl_new_interest\n");
			     ...
			     struct ccnl_face_s *from)
{
    ...
    fprintf(stderr, "ccnl_append_pending\n");

    for (pi = i->pending; pi; pi = pi->next) { // check whether already listed
	if (pi->face == from) {
	    fprintf(stderr, "  we found a matching interest, updating time\n");
    ...
    fprintf(stderr, "  appending a new pendint entry %p\n", (void *) pi);
    if (!pi)
	return -1;
    pi->face = from;
    pi->last_used = CCNL_NOW();
...
ccnl_interest_propagate(struct ccnl_relay_s *ccnl, struct ccnl_interest_s *i)
{
    ...
    fprintf(stderr, "ccnl_interest_propagate\n");
	...
	fprintf(stderr, "  ccnl_interest_propagate, rc=%d/%d\n", rc, fwd->prefix->compcnt);
	if (rc < fwd->prefix->compcnt)
	    continue;
	fprintf(stderr, "  ccnl_interest_propagate, fwd==%p\n", (void*)fwd);
...
ccnl_interest_remove(struct ccnl_relay_s *ccnl, struct ccnl_interest_s *i)
{
    ...
    fprintf(stderr, "ccnl_interest_remove %p\n", (void *) i);
...
{
    unsigned char *md;
    fprintf(stderr, "ccnl_i_prefixof_c prefix=%s min=%d max=%d\n",
		 ...
		 unsigned char *content, int contlen)
{
    ...
    fprintf(stderr, "ccnl_content_new <%s>\n",
...
ccnl_content_remove(struct ccnl_relay_s *ccnl, struct ccnl_content_s *c)
{
    ...
    fprintf(stderr, "ccnl_content_remove\n");
...
{
    fprintf(stderr, "ccnl_content_add2cache (%d/%d)\n",
...
ccnl_content_serve_pending(struct ccnl_relay_s *ccnl, struct ccnl_content_s *c)
{
    ...
    struct ccnl_face_s *f;
    int cnt = 0;
    fprintf(stderr, "ccnl_content_serve_pending\n");
	...
	for (pi = i->pending; pi; pi = pi->next) {
	    ...
	    if (pi->face->ifndx >= 0) {
		fprintf(stderr, "  forwarding content <%s>\n",
...
ccnl_do_ageing(void *ptr, void *dummy)
{
    ...
    time_t t = CCNL_NOW();
    fprintf(stderr, "ccnl_do_ageing %d\n", (int)t);
...
{
    int k;
    fprintf(stderr, "ccnl_core_cleanup %p\n", (void *) ccnl);
		    ...
		    unsigned char **data, int *datalen)
{
    ...
    fprintf(stderr, "ccnl_core_RX_i_or_c (%d bytes left)\n", *datalen);

    buf = ccnl_extract_prefix_nonce_ppkd(data, datalen, &scope, &aok, &minsfx,
			 &maxsfx, &p, &nonce, &ppkd, &content, &contlen);
    if (!buf) {
	    fprintf(stderr, "  parsing error or no prefix\n"); goto Done;
    }
    if (nonce && ccnl_nonce_find_or_append(relay, nonce)) {
	fprintf(stderr, "  dropped because of duplicate nonce\n"); goto Skip;
    }
	...
	if ( aok & 0x01 ) { // honor "answer-from-existing-content-store" flag
		...
		fprintf(stderr, "  matching content for interest, content %p\n", (void *) c);
	...
	if (!i) { // this is a new/unknown I request: create and propagate
		...
		fprintf(stderr, "  created new interest entry %p\n", (void *) i);
		if (scope > 2)
		    ccnl_interest_propagate(relay, i);
	    }
	} else if (scope > 2 && (from->flags & CCNL_FACE_FLAGS_FWDALLI)) {
	    fprintf(stderr, "  old interest, nevertheless propagated %p\n", (void *) i);
	    ccnl_interest_propagate(relay, i);
	}
	if (i) { // store the I request, for the incoming face (Step 3)
	    fprintf(stderr, "  appending interest entry %p\n", (void *) i);
	...
	if (c) { // CONFORM: Step 2 (and 3)
		...
		fprintf(stderr, "  removed because no matching interest\n");
		free_content(c);
		goto Skip;
	    }
	    if (relay->max_cache_entries != 0) { // it's set to -1 or a limit
		fprintf(stderr, "  adding content to cache\n");
		ccnl_content_add2cache(relay, c);
	    } else {
		fprintf(stderr, "  content not added to cache\n");
		free_content(c);
	    }
	}
    }
...
{
    int rc = 0, num, typ;
    fprintf(stderr, "ccnl_core_RX_datagram: %d bytes from face=%p (id=%d.%d)\n",
...
#endif
	default:
	    fprintf(stderr, "  unknown datagram type %d\n", num);
	    return -1;
	}
    }
    return rc;
...
{
    struct ccnl_face_s *from;
    fprintf(stderr, "ccnl_core_RX ifndx=%d, %d bytes\n", ifndx, datalen);
