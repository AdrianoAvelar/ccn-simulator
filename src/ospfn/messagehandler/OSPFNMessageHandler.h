...
public:
    MessageHandler(OSPFN::Router* containingRouter, cSimpleModule* containingModule);

//
    void    messageReceived(cMessage* message);
    void    handleTimer(OSPFNTimer* timer);
//
    //void    processPacket(OSPFNPacket* packet, Interface* unused1 = NULL, Neighbor* unused2 = NULL);
    virtual void    processPacket(OSPFNPacket* packet);
    void    send(cPacket * packet);

    void    sendPacket(OSPFNPacket* packet, string destination, int outputIfIndex, short ttl = 1);
//    void    sendPacket(OSPFNPrefixAdv* packet);
      void    clearTimer(OSPFNTimer* timer);
      void    startTimer(OSPFNTimer* timer, simtime_t delay);
      cSimpleModule* getOSPFNModule(){return ospfModule;};
