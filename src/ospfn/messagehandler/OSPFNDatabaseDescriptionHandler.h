...
private:
    bool processDDPacket(OSPFNDatabaseDescriptionPacket* ddPacket);//, Interface* intf, Neighbor* neighbor, bool inExchangeStart);

public:
    DatabaseDescriptionHandler(Router* containingRouter);

    void processPacket(OSPFNPacket* packet);//, Interface* intf, Neighbor* neighbor);
    //void processPacket(OSPFNPacket* packet, Interface* intf, Neighbor* neighbor);
