...
typedef std::map<MACAddress,pTableEntry> tableEntry;
typedef std::map<chunk_t,tableEntry> prefixTable_map;

typedef struct PrefixEntry_{
    chunk_t prefix;
    MACAddress src;
    MACAddress dst;
    int hops;
...
typedef std::vector<PrefixEntry> PrefixVector;
typedef boost::unordered_map<chunk_t, PrefixVector> PrefixMap;
...
private:
      std::string                                                           routerName;                ///< The router ID assigned by the IP layer.
...
//    Router(RouterID id, cSimpleModule* containingModule);
    Router(std::string id, cSimpleModule* containingModule);
//    /**
//     * Destructor.
...
//     */
    ...
    bool existEntryInPF(chunk_t prefix, int hops);
    int getQntOfPrefixEntry(chunk_t prefix);
    void dumpInterfaces();

    void addInterface(OSPFN::Interface* intf);
    string getPacketTypeName(int type);
    bool floodHello();
    double lastFlood;//Avoid flooding all the time.
    std::string getRouterID(){return routerName;};
    OSPFN::Interface* getInterfaceByMac(MACAddress &macaddress);
    void dumpPrefixTable();
    std::vector<chunk_t> getDesiredPrefix(std::vector<chunk_t> prefix_list);
    void addToPrefixTable(string prefix, MACAddress src, MACAddress dest, bool isLocal, int hops, bool isAdv );
    void addOrUpdatePrefixTableEntry(string prefix, MACAddress src, MACAddress dest, bool isLocal, int hops, bool isAdv);

    void setForwardingTable(ForwardingTable* ft);
    ForwardingTable* getForwardingTable();

     std::vector<adv_prefix> getPrefixesToAdvertise(std::vector<chunk_t> lsa_req_v);
     std::vector<adv_prefix> getPrefixesToAdvertise();
     void clearNeighborhood();
     void clearPrefixTable();
//
...
//
//
//    PrefixList getPrefixToAdvertise();
//    void addNeighborPrefix(Prefix *prefix, int size,  IPv4ControlInfo* );
//    void rebuildForwardingTable();
//    //OSPFNPrefixAdv* rebuildForwardingTable();
//
//    void addPrefixToRouter(std::string prefixName);
//
//    struct next_hop_s{
//            std::string prefix;
//            MACAddress advertisingRouter;
//            MACAddress accessFrom;
//            simtime_t time;
//            int cost;
//        };
//
//    typedef std::map<std::string,next_hop_s> NamePrefixTable;
//    NamePrefixTable namePrefixTable;
//    PrefixList myPrefixList;
//    /**
