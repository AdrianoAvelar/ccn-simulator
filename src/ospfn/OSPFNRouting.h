 ...
 **/
class OSPFNRouting :  public cSimpleModule, public ICcnApp, public cListener
{
  private:
    IInterfaceTable* ift;        ///< Provides access to the interface table.
    ForwardingTable*  ft;         ///< Provides access to the IP routing table.
    OSPFN::Router*    ospfnRouter; ///< Root object of the OSPF data structure.
    int appId;
    Ccn* ccnLayer;
...
//
  public:
    OSPFNRouting();
    virtual ~OSPFNRouting();

    void sendPacket(cPacket* packet);
    OSPFN::Router* getRouter();

    virtual void registerMyselfInCcnLayer();
    simtime_t getCurrentTime(){return simTime();}
