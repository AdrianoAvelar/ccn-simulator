...
RESULT_DIR=../plots
#STAT_VECTOR=("caching*" "hit_rate*" "hit_hop_*" "psnr_*" "ssim_*" "rtt_all_pkts_*" "replacements" )
STAT_VECTOR=("hitRate*" "hitHop*" "rttAll*")
#STAT_SCALAR=("server_hit" "int_delay_*" "obj_delay_*" "tot_mean_delay")
#STAT_SCALAR=("server_hit" "tot_mean_delay" "dropped_*" )
#STAT_SCALAR=("dropped_*" )
STAT_SCALAR=("pkts_dropped")
...
function calculate_vec() {
	...
	policy=$1/$2
	echo "Vector: Calculation ...  $policy # metric: $3"
	scavetool vector -p "name($3)" -O out $policy/ccn-id.vec
	#Delete line which doesn't start with numeric
	if [ ! -e "out.vec" ]; then
  		echo "$3 not exists in $policy "
		return
	fi
	...
	awk '{sum+=$4; array[NR]=$4} END {for(x=1;x<=NR;x++){sumsq+=((array[x]-(sum/NR))**2);} print sum/NR};' output.vec | tee -a $RESULT_DIR/$1_$3.txt
 ...
 echo "*** ${STAT_VECTOR[$j]} ***"
	for (( i = 0 ; i < ${#DIRS_VEC[@]} ; i++ )) do
		...
		for (( k = 0 ; k < ${#SUBDIR_VEC[@]} ; k++ )) do
			calculate_vec ${DIRS_VEC[$i]} ${SUBDIR_VEC[$k]} ${STAT_VECTOR[$j]}
		done
...
echo "*** ${STAT_SCALAR[$j]} ***"
	for (( i = 0 ; i < ${#DIRS_VEC[@]} ; i++ )) do
		...
		for (( k = 0 ; k < ${#SUBDIR_VEC[@]} ; k++ )) do
			calculate_sca ${DIRS_VEC[$i]} ${SUBDIR_VEC[$k]} ${STAT_SCALAR[$j]}
		done
